...
#define nrn_init _nrn_init__ExpSynCaSTDP
#define _nrn_initial _nrn_initial__ExpSynCaSTDP
#define nrn_cur _nrn_cur__ExpSynCaSTDP
#define _nrn_current _nrn_current__ExpSynCaSTDP
#define nrn_jacob _nrn_jacob__ExpSynCaSTDP
#define nrn_state _nrn_state__ExpSynCaSTDP
#define _net_receive _net_receive__ExpSynCaSTDP 
#define state state__ExpSynCaSTDP 
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 6, "fornetcon");
 ...
 add_nrn_fornetcons(_mechtype, _fnc_index);
 	...
 	ivoc_help("help ?1 ExpSynCaSTDP /Users/shkim/Research/neurcomp/Neural-information-processing-and-computations-of-two-input-synapses/Fig2BandFig3/x86_64/stdp_ca.mod\n");
...
{  double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt;
 ...
 {
   if ( _lflag  == 0.0 ) {
     printf ( "entryca flag=%g t=%g w=%g A=%g tpre=%g tpost=%g g=%g ca=%g ca_thresh=%g\n" , _lflag , t , _args[0] , _args[1] , _args[2] , tpost , g , cai , ca_thresh ) ;
     printf ( "wsynca=%g\n" , _args[3] ) ;
       if (nrn_netrec_state_adjust && !cvode_active_){
    /* discon state adjustment for cnexp case (rate uses no local variable) */
    double __state = g;
    double __primary = (g + _args[3]) - __state;
     __primary += ( 1. - exp( 0.5*dt*( ( - 1.0 ) / tau ) ) )*( - ( 0.0 ) / ( ( - 1.0 ) / tau ) - __primary );
    g += __primary;
  } else {
 g = g + _args[3] ;
       }
 _args[1] = _args[1] * exp ( ( _args[2] - t ) / ptau ) + dp ;
     _args[2] = t ;
     _args[3] = _args[3] + wmax * D * exp ( ( tpost - t ) / dtau ) ;
     if ( _args[3] < wmin ) {
       _args[3] = wmin ;
       }
     }
 ...
 {
       _fnargs1[3] = _fnargs1[3] + wmax * _fnargs1[1] * exp ( - ( t - _fnargs1[2] ) / ptau ) ;
       if ( _fnargs1[3] > wmax ) {
         _fnargs1[3] = wmax ;
         }
       }
...
#if NMODL_TEXT
static const char* nmodl_filename = "/Users/shkim/Research/neurcomp/Neural-information-processing-and-computations-of-two-input-synapses/Fig2BandFig3/mod_files/stdp_ca.mod";
static const char* nmodl_file_text = 
